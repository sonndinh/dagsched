...
class AdjListDrawing(Frame):
    def __init__(self, num_subtasks, adj_list, crit_path, wcets, span, deadline, period, master=None):
    ...
    def show(self):
        # 4 heading lines plus 1 line for each subtask.
        num_lines = 4 + int(self.num_subtasks)
        ...
        for node_len in self.wcets:
            ...
            wcets_line += "v" + str(sub_id) + "=" + node_len + " "
            sub_id += 1
        wcets_line += "\n"
        ...
        # A line for critical-path.
        crit_path_line = "Critical-path: "
        for sub_id in self.crit_path:
            crit_path_line += "v" + str(sub_id) + " "
        crit_path_line += "\n"
        self.app.insert(END, crit_path_line)
...
class Entry:
    ...
    def read_dag(self, dag_path):
        ...
        # Third line: <work> <span> <deadline> <period>
        parameters = dag_f.readline().split()
        self.span = parameters[1]
        self.deadline = parameters[2]
        self.period = parameters[3]
        # Fourth line: critical-path
        self.crit_path = dag_f.readline().split()
    ...
    def read_proposed_sched(self, proposed_path):
        ...
        # Second line: <Number of processors> <Schedule finish time>
        tokens = f.readline().split()
        no_procs = int(tokens[0])
    ...
    def run(self):
        ...
        self.dag = AdjListDrawing(self.num_subtasks, self.adj_list, self.crit_path, self.wcets, \
        ...
        self.scheds.finalize()
        self.scheds.pack(fill="both", expand=True)
        root.mainloop()
